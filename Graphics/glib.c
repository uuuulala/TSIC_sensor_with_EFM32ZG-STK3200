...
EMSTATUS GLIB_contextInit(GLIB_Context_t *pContext) {
	...
	GLIB_Rectangle_t tmpRect = { 0, 0, pTmpDisplayGeometry->xSize - 1,
			pTmpDisplayGeometry->ySize - 1 };
...
EMSTATUS GLIB_setClippingRegion(GLIB_Context_t *pContext,
	...
	if ((pRect->xMin < pContext->pDisplayGeometry->xClipStart)
			...
			|| (pRect->yMax > pContext->pDisplayGeometry->clipHeight - 1))
		return GLIB_OUT_OF_BOUNDS;

	/* Set clipping region in driver */
	status = DMD_setClippingArea(pRect->xMin, pRect->yMin,
			pRect->xMin + pRect->xMax + 1, pRect->yMin + pRect->yMax + 1);

	GLIB_Rectangle_t tmpRect = { pRect->xMin, pRect->yMin, pRect->xMax,
			pRect->yMax };
...
EMSTATUS GLIB_resetClippingRegion(GLIB_Context_t *pContext) {
	...
	pContext->clippingRegion.xMax = pContext->clippingRegion.xMin
			+ pContext->pDisplayGeometry->clipWidth - 1;
	pContext->clippingRegion.yMax = pContext->clippingRegion.yMin
			+ pContext->pDisplayGeometry->clipHeight - 1;
